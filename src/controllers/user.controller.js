...
const changeCurrentPassword = asyncHandler(async (req, res) => {
    ...
    if (await user.isPasswordCorrect(oldpassword)) {
        ...
        return res.status(200).
            json(ApiResponse(200, "password Successfully changed!!"));
    } else {
...
const updateAccountDetails = asyncHandler(async (req, res) => {
    ...
    if (!fullName || !email) {
        throw new ApiError(400, "both fullname and email are required fields");
    }
    ...
    return res.status(200).
        json(
            new ApiResponse(200, user, "accounted details updated succesfully")
        );
});
...
const getUserChannelProfile = asyncHandler(async (req, res) => {
    ...
    console.log(channel);
    if (!channel?.length) {
        throw new ApiError(40, "channel could not be found");
    }
...
const getUserWatchHistory = asyncHandler(async (req, res) => {
    ...
    if(!watchedVideos){
        throw new ApiError(400,"failed fetching watch history!!");
    }
    return res.status(200).
    json(
        new ApiResponse(200,watchedVideos[0].watchedVideos,"watch history fetched successfully!!!")
    );
});
